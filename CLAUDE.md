...
| `renderWelcomeNote(name, team)` | 소속팀만 있을 때 `WELCOME_TEMPLATE`의 `{{이름}}`/`{{팀}}`을 치환해 환영 인사 문구 교체 |
| `renderWelcomeSign(team)` | 환영 인사 서명(`— LX인터내셔널 OOO팀 일동`)만 갱신. 환영메시지가 있어도 서명은 항상 이 함수로 통일 |
...
**"동료들의 환영 인사" 문구는 3단계로 결정됩니다** (Sheet 열: `소속팀`, `환영메시지`):
1. 둘 다 비어 있음 → HTML의 기본 범용 문구 그대로
2. 소속팀만 있음 → `WELCOME_TEMPLATE`에 이름·팀명을 치환한 문구
3. `환영메시지`가 채워져 있음 → 그 문구를 그대로 사용 (치환 없이 원문 그대로 노출되므로,
   이 열에 입력할 때 이름을 이미 넣어서 작성해야 함). 서명(`— LX인터내셔널 OOO팀 일동`)은
   이 경우에도 `소속팀` 값으로 자동 생성됨
