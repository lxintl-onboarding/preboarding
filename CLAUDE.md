...
**다음 작업**: 레이아웃·비주얼 개선(하단 네비게이션·SVG 아이콘·타이포 정리)과 "입사 후"
탭의 "입사 2주 이내 체크사항"(사원증 촬영·법인카드·개인정보 입력 등) 콘텐츠 반영은
완료된 상태입니다. 다음 우선순위는 남은 콘텐츠 보강(팀/동료 환영 메시지, FAQ,
...
3. **입사 후** *(D-Day 당일부터 노출, 5탭 중 정가운데 위치)* — `#tab-after`/`#panel-after`.
   ...
   "입사 2주 이내 체크사항"(인사팀 제공 실제 콘텐츠, [입사 2일 이내] / [입사 10일 이내]
   두 그룹, 총 8개 항목)을 `.after-todos` 체크리스트 2개로 보여주고, 그 아래
   "[참고] 법인카드 안내" / "[참고] 관련 문의"를 아코디언(`.acc`, Tab5 컴포넌트 재사용)으로
   덧붙임 (별도 localStorage 키, `#docsTodo`처럼 토큰별 분리). 노출되는 순간 다른 4탭(흰 배경·붉은
