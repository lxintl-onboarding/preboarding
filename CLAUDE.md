...
- `환영메시지`만 예외입니다 — 팀으로부터 나중에 따로 받는 값이라 비어 있을 수 있고,
  비어 있으면 본문은 HTML의 기본 범용 문구를 그대로 씁니다. `환영메시지`는 이름/소속팀을
  본문 안에 다시 언급할 필요 없이(이미 To/From에 자동으로 표시됨) 순수 메시지 내용만
  적으면 됩니다. 줄바꿈은 그대로 반영됩니다(`white-space: pre-line`).
